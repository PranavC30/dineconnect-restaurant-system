...
const GoogleLogin = ({ onSuccess, onError }) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const fallbackButton = document.getElementById('google-fallback');
      if (fallbackButton) {
        fallbackButton.style.display = 'flex';
        console.log('✅ Google button shown');
      }
    }, 500);
  }, [onSuccess, onError]);
  ...
  const handleGoogleLoginClick = () => {
    console.log('🔍 Google login button clicked - Opening account selector');
    setShowAccountSelector(true);
  };

  // Handle account selection from popup
  const handleAccountSelect = (userData) => {
    console.log('🎯 Account selected:', userData.email);
    
    // Call success callback with selected user data
    if (onSuccess) {
      onSuccess(userData);
    }
  };
  ...
  const handlePopupClose = () => {
    console.log('❌ Google account selector closed');
